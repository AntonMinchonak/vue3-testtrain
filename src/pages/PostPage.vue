...
<template>
  <div class="postsPage" @scroll="scrolled">
    ...
    <PostList @changeSort='changeSort' @deletePost='deletePost' :sort='sort' :posts="searchedPosts" />
    <h4 v-if="isDeleted">Post was succesfully deleted</h4>
    <h4 v-if="!searchedPosts.length">List is empty</h4>
    <!-- <div ref='scrollBorder' class="scroll-border"></div> -->
    <div v-intersection="{page, isLoading, fetchPosts}" class="scroll-border"></div>
  </div>
</template>
